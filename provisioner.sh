...
cd $RUNNER_DIR/charts/ingress-nginx
helm upgrade --debug --install --create-namespace ingress-controller -n ingress-controller --set controller.kind=DaemonSet --set controller.healthStatus=true  --set controller.healthCheckPath="/healthz" --set controller.ingressClass=nginx-controller   --set controller.service.type=NodePort --set controller.service.nodePorts.http=32038  --set controller.service.nodePorts.https=32034 --set metrics.prometheusRule.enabled=true --set controller.service.customPorts[0].port=9113 --set controller.service.customPorts[0].targetPort=9113 --set controller.service.customPorts[0].protocol=TCP --set controller.service.customPorts[0].name=ingress-prometheus --set controller.service.customPorts[0].nodePort=31040 --set-string controller.config.entries.use-forward-headers=true,controller.config.entries.compute-full-forwarded-for=true,controller.config.entries.use-proxy-protocol=true .
#helm upgrade --debug --install --create-namespace ingress-controller -n ingress-controller --set controller.kind=Daemonset --set controller.healthStatus=true --set controller.healthStatusURI="/healthz" --set controller.ingressClass=nginx-controller  --set controller.service.type=NodePort --set controller.service.nodePorts.http=32038 --set controller.service.nodePorts.https=32034 --set prometheus.create=true --set controller.service.customPorts[0].port=9113 --set controller.service.customPorts[0].targetPort=9113 --set controller.service.customPorts[0].protocol=TCP --set controller.service.customPorts[0].name=ingress-prometheus --set controller.service.customPorts[0].nodePort=31040 --set-string controller.config.entries.use-forward-headers=true,controller.config.entries.compute-full-forwarded-for=true,controller.config.entries.use-proxy-protocol=true .
